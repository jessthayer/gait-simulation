...
param.note = 'Thayer Subject 1, 2019';

%input anthropometric measurements
param.gender = 'male';
param.age = '26';
param.height = 1.8288; % (m)
param.weight = 85.729; % (kg)
param.orthosisWeight = 1; % (kg)
param.legLength = 0.81; %ASIS to lateral malleoli (m)
param.thighLength = 0.40; %greater trochanter to lateral femoral epicondyle (m)
param.kneeDiam = 0.102; %knee diameter (m)
param.ankleDiam = 0.0665; %ankle diameter (m)
param.interASIS = 0.30; %ASIS to ASIS (m)
param.thighProx = 0.627; %thigh proximal circumference (m)
param.thighDist = 0.435; %thigh distal circumference (m)
param.malHeight = 0.095; %height of malleolus\ankle joint from ground (m)
param.footLength = 0.27; %length of foot, measured from AFO (m)
param.rockerLength = 0.20; %length from heel to ball of foot, measured from AFO (m)
param.footWidth = 0.09; %width of ball of foot, measured from AFO (m)
