...
global ss_predictionHorizon ds_predictionHorizon controlWindow timeStep...
    ...
    complete_ds2 complete_ss2 complete_ds2

cd([sim_file_dir '\' trialName]) %check directory location before creating new folder

mkdir('SS2') %create a folder for gait period
subdirectory = cd('SS2'); %open new folder
...
%open internal and plant models, set stopTime, save to subdirectory
open_system(ss_inter_loc);
save_system(ss_inter_loc, 'ss2_inter');
set_param('ss2_inter','StopTime',num2str(ss_predictionHorizon));
save_system('ss2_inter');

open_system(ss_plant_loc);
save_system(ss_plant_loc, 'ss2_plant');
set_param('ss2_plant','StopTime',num2str(ss_predictionHorizon));
save_system('ss2_plant');
...
load([sim_file_dir '\' trialName '\ds2\ds2_plant_end'])
...
save('ss2_inter_iter_1'); %save internal model iteration

for ii = 1:7
    u(ii,:) = pOpt(ii).Value;
end

save('ss2_optimizedcontrolinput_iter_1','u'); %save optimized control inputs
...
save('ss2_plant_iter_1'); %save plant iteration

%save video of initial simulation
video = [sim_file_dir '\' trialName '\ss2_sim_initial'];
...
for i = 1:(floor(ss_predictionHorizon/controlWindow) - 1)
    ...
    load(['ss2_optimizedcontrolinput_iter_' num2str(i)]); %set MPC design vars
    ...
    load(['ss2_nextoptIC_iter_' num2str(i)]);
    ...
    save(['ss2_inter_iter_' num2str(i+1)]) %save internal model iteration
    
    for ii = 1:4
        u(ii,:) = pOpt(ii).Value;
    end
    
    filename = ['ss2_optimizedcontrolinput_iter_' num2str(i+1)];
    ...
    plant = 'ss2_plant'; %load plant
    
    %load initial conditions for next optimization
    load(['ss2_optimizedcontrolinput_iter_' num2str(i+1)]);
    load(['ss2_controlinputhistory_iter_' num2str(i)]);
    ...
    save(['ss2_plant_iter_' num2str(i+1)]); %save plant iteration
    ...
    filename = ['ss2_nextoptIC_iter_' num2str(i+1)];
    save(filename,'Planar_joint_x_p_out1','Planar_joint_x_v_out1','Planar_joint_y_p_out1','Planar_joint_y_v_out1','Planar_joint_z_p_out1','Planar_joint_z_w_out1','stance_ankle_p_out1','stance_ankle_w_out1','stance_hip_p_out1','stance_hip_w_out1','stance_knee_p_out1','stance_knee_w_out1','swing_ankle_p_out1','swing_ankle_w_out1','swing_hip_p_out1','swing_hip_w_out1','swing_knee_p_out1','swing_knee_w_out1');

    filename = ['ss2_controlinputhistory_iter_' num2str(i+1)];
...
filename = 'ss2_nextoptIC_end';
save(filename,'Planar_joint_x_p_out1','Planar_joint_x_v_out1','Planar_joint_y_p_out1','Planar_joint_y_v_out1','Planar_joint_z_p_out1','Planar_joint_z_w_out1','stance_ankle_p_out1','stance_ankle_w_out1','stance_hip_p_out1','stance_hip_w_out1','stance_knee_p_out1','stance_knee_w_out1','swing_ankle_p_out1','swing_ankle_w_out1','swing_hip_p_out1','swing_hip_w_out1','swing_knee_p_out1','swing_knee_w_out1');

filename = 'ss2_controlinputhistory_end';
save(filename,'u_stance_ankle_data','u_stance_knee_data','u_stance_hip_data','u_swing_ankle_data','u_swing_knee_data','u_swing_hip_data');
