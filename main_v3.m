...
objfun_vers = 'v0'; %version of cost functions to use
inter_vers = 'v0'; %version of internal models to use
plant_vers = 'v0'; %version of plant models to use
...
try
    ...
    if exist('complete_ds1','var') == 0
        ...
        run ds1_v2 % run double support period 1
        complete_ds1 = 1;
        cd(subdirectory)
    end
    ...
    if exist('complete_ss1','var') == 0
        addpath(sim_file_dir)
        run ss1_v2 % run double support period 1
        complete_ss1 = 1;
        cd(subdirectory)
    end   
    ...
    if exist('complete_ds2','var') == 0
        addpath(sim_file_dir)
        run ds2_v2 % run double support period 1
        complete_ds2 = 1;
        cd(subdirectory)
    end
    ...
    if exist('complete_ss2','var') == 0
        addpath(sim_file_dir)
        run ss2_v2 % run double support period 1
        complete_ss2 = 1;
        cd(subdirectory)
    end         
